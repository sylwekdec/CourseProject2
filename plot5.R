...
data1<-data[grep("Vehicle", data$Short.Name),]
...
ggplot(data = totyear, aes(x=Year, y=Emission))+geom_bar(colour="black", fill="red", stat="identity")+ggtitle('Total Emissions from Motor Vehicle sources from 1999 to 2008')+ylab(expression('PM'[2.5]*" Emissions"))
dev.off()
